...
public class TridentSpoutWrapper extends BaseWrapper<ITridentSpout> implements ITridentSpout {

    protected static final Logger LOG = LoggerFactory.getLogger(TridentSpoutWrapper.class);

    public TridentSpoutWrapper(Topology topology, TopologyComponent component,
            boolean isCluster, StreamflowConfig configuration) throws FrameworkException {
        super(topology, component, isCluster, configuration, ITridentSpout.class);
    }
    ...
    public BatchCoordinator getCoordinator(String txStateId, Map conf, TopologyContext context) {
        try {
            return getDelegate().getCoordinator(txStateId, conf, context);
        } catch (FrameworkException ex) {
    ...
    public Emitter getEmitter(String txStateId, Map conf, TopologyContext context) {
        try {
            return getDelegate().getEmitter(txStateId, conf, context);
        } catch (FrameworkException ex) {
